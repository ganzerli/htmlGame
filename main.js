...
function setPosition(sprite){ //renders objects in DOM
...
function toggleKey(keyCode, isPressed){ //every pressed key check
	switch(keyCode) {
	    case UP_KEY:
...
function checkCollision(){
	for(let i = 0 ; i<enemysArray.length; i++){
		...
		}else if(enemysArray[i].y > 500){ // if one is ver the bottom/screen
			let foundElement= document.getElementById(enemysArray[i].element); 
			foundElement.style.opacity = 0;
			foundElement.parentNode.removeChild(foundElement);
			enemysArray.splice(i,1);
...
function updatePositions(){// of anithing without control over
...
function loop(){
	let lastLoopRun =0;
	if(new Date().getTime() - lastLoopRun > 40){// loops every 40 MilliSeconds.
...
let hero = createObject('hero',250,460,20,20);
